...
int main(int argc, char *argv[]){
    ...
    std::function<double (double)> freqOsc = std::bind(Waveforms::sine, 2, std::placeholders::_1);

    Instrument e4;
    e4.wave = std::bind(Waveforms::modulatedWave, 330, std::placeholders::_1, 0.01f, freqOsc, Waveforms::square);
    e4.amplitude = 3000;
    e4.envelope = &e4Envelope;
    
    Instrument f4;
    f4.wave = std::bind(Waveforms::modulatedWave, 350, std::placeholders::_1, 0.01f, freqOsc, Waveforms::square);
    f4.amplitude = 3000;
